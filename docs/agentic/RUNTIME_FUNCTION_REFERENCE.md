...
| `scripts/skillops.mjs` | `main()` | repo-local SkillOps CLI: debrief, non-durable distill, promotion planning/apply/mark |
...
- `SkillOpsPromotionTaskError`: structured promotion-lane failure wrapper with explicit `reasonCode`.
...
- SkillOps promotion state helpers:
  ...
  - `getSkillOpsPromotionPlanPath(...)`
  - `getSkillOpsPromotionStatePath(...)`
  - `getSkillOpsPromotionLockPath(...)`
  - `buildSkillOpsPromotionTaskId(...)`
  - `buildSkillOpsPromotionBranchName(...)`
  - `buildSkillOpsPromotionCurationWorkdir(...)`
...
- `inferUserRequestedReviewGate(...)`: infer review gate from user request text; explicit commit selectors come only from directive-shaped review lines in the newest update body when a newer update exists, while initial requests without updates still honor directive-shaped selectors from the current title/body and title/full body still preserve review intent and PR reference. Short SHAs must uniquely resolve against the PR commit list when a PR is targeted. On non-PR requests, short-SHA targets and explicit excludes resolve locally before validation, exclude-only narrowing applies against the resolved target set instead of collapsing to a dirty-tree review, and explicit include/exclude filters fail closed when the target cannot be resolved or every resolved target is excluded.
...
- SkillOps promotion helpers used by the main loop:
  ...
  - `resolveSkillOpsPromotionBase(...)`: resolve repo default branch with `origin/HEAD -> origin/main -> origin/master -> HEAD`
  - `readSkillOpsPromotionState(...)`: read deterministic promotion state file
  - `acquireSkillOpsPromotionLock(...)`: stale-safe shared curation worktree lock
  - `ensureSkillOpsPromotionCurationWorkdir(...)`: create/validate the shared curation worktree
...
- `attemptStaleWorkerWorktreeReclaim(...)`: sync reclaim helper for stale dirty worker worktrees; only runs when the agent has no other open tasks, the incoming contract is deterministic, stale ownership is proven by both old-root mismatch and current-branch mismatch, and the dirt is not on the controller-housekeeping path for pending SkillOps promotion material.
