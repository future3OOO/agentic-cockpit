...
- When autopilot itself closes a root `blocked`, runtime plans a same-root recovery continuation, closes the source receipt first, then queues the recovery task; controller-remediable gate reasons stay in auto-recovery alert mode, while external blockers still use bounded retries, and any post-close enqueue failure is flushed from a deterministic pending marker on the next poll instead of mutating the source receipt.
