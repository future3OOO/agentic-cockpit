...
- For `dirty_cross_root_transition`, runtime reruns the shared dirt classifier before it commits to blocked recovery:
  - pure controller-owned recoverable dirt stamps a controller-class contract and is diverted into runtime-owned `controller-housekeeping`,
  - mixed/model-authored dirt still falls through to ordinary external blocked-recovery handling.
...
- Controller-housekeeping is runtime-only:
  - runtime snapshots the blocked task, persists housekeeping state, moves focus to the synthetic housekeeping root, and only then closes the source task `blocked/controller_housekeeping_pending`,
  - housekeeping packets use `signals.kind=ORCHESTRATOR_UPDATE`, `signals.sourceKind=AUTOPILOT_CONTROLLER_HOUSEKEEPING`, `signals.phase=controller-housekeeping`, and `signals.parentId=<first suspended original task id>`,
