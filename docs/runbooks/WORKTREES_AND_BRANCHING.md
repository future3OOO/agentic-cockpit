...
- Idle worker worktrees should be repinned back to their roster branch after root closure; stale non-roster branches are runtime reclaim targets only when the agent has no `new`/`seen`/`in_progress` follow-up packets, not permanent parking spots.
