...
export async function runPostMergeResync({
  ...
  try {
    ...
    for (const target of targets) {
      ...
      if (target.workdir === path.resolve(projectRoot)) {
        result.repin.skipped += 1;
        result.repin.skippedReasons.push(`${target.name}:project_root_workdir`);
        continue;
      }
      if (target.branch.startsWith('runtime/')) {
        result.repin.skipped += 1;
        result.repin.skippedReasons.push(`${target.name}:runtime_branch_excluded`);
        continue;
      }
